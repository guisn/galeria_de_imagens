...
class ManipuladorDeImagens {
    ...
    public static function modificaTamanhoDaImagem($nome_img, $lar_maxima, $alt_maxima, $qualidade = 100) {
        ...
        # Pega onde estс a imagem e carrega	 
        if ($tipo == 2) { // 2 щ o JPG
        ...
        if ($img) {
            // Pega o tamanho da imagem e proporчуo de resize
            $width = imagesx($img);
            $height = imagesy($img);
            $scale = min($lar_maxima / $width, $alt_maxima / $height);

            // Se a imagem щ maior que o permitido, encolhe ela!
            if ($scale < 1) {
                $new_width = floor($scale * $width);
                $new_height = floor($scale * $height);

                // Cria uma imagem temporсria
